...
acf_traveltime <- acf(uberTrain$MeanTravelTimeSeconds, na.action = na.pass, lag.max=100, plot=FALSE)
plot(acf_traveltime, main="ACF-Mean Travel Time")
acf_windspeed <- acf(uberTrain$windspeed, na.action = na.pass, lag.max=100, plot=FALSE)
...
# 5 NA values in tmin, 6 NA values in tmax and 1 NA values in the prcp column
...
# P-values indicate the data is non-stationary
...
travel_times_test <- ts(uberTest$MeanTravelTimeSeconds, start = end(travel_times_train), frequency=7)
